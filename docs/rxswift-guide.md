...
이 문서에서는 `Observable`, Subject, Relay, Trait와 `subscribe`, `bind`, `drive`, `emit`의 차이를 설명해요. 마지막 선택표에서는 상황별로 어떤 타입과 메서드를 사용해야 하는지 바로 확인할 수 있어요.
...
| 구분 | 동작 | 대표 예시 |
| --- | --- | --- |
| Cold Observable | 구독할 때마다 작업을 새로 시작해요. | 네트워크 요청, `Observable.create`, `Single` |
...
```swift
let alertRelay = PublishRelay<MapBoxAlert>()
...
```swift
let coordinate: Single<MapBoxCoordinate> = provider.requestLocation()
...
## RxCocoa Trait는 UI에 필요한 규칙을 보장해요

`Observable`은 실행 스레드, 오류, 이벤트 공유 방법을 직접 결정해야 해요. RxCocoa Trait는 UI에서 자주 필요한 규칙을 타입으로 표현해요.
...
현재 프로젝트의 `rx.viewDidAppear`와 `rx.viewDidDisappear`도 `ControlEvent<Void>`예요.
...
```swift
let alert: Signal<MapBoxAlert> = alertRelay.asSignal()
...
| `flatMapLatest` | 기존 작업을 취소하고 최신 작업만 유지해요. | 검색어 자동 완성 |
...
현재 `MapBoxFeatureViewModel`은 위치 요청 중 연속 탭을 무시하기 위해 `flatMapFirst`를 사용해요.
...
## MapBoxFeature에서는 이렇게 사용해요

| 위치 | 타입 | 이유 |
| --- | --- | --- |
| `locationButton.rx.tap` | `ControlEvent<Void>` | UIKit 버튼 입력이기 때문이에요. |
| `rx.viewDidAppear` | `ControlEvent<Void>` | 화면 생명주기 이벤트이기 때문이에요. |
...
| `cameraCommandRelay` | `PublishRelay<MapBoxCameraCommand>` | 내부에서 일회성 카메라 명령을 발생시켜요. |
| Output `cameraCommand` | `Signal<MapBoxCameraCommand>` | 과거 카메라 명령을 재실행하지 않아요. |
| `alertRelay` | `PublishRelay<MapBoxAlert>` | 내부에서 일회성 알림을 발생시켜요. |
| Output `alert` | `Signal<MapBoxAlert>` | 이전 알림을 다시 표시하지 않아요. |
| `requestLocation()` | `Single<MapBoxCoordinate>` | 위치를 한 번 성공하거나 오류로 종료해요. |
